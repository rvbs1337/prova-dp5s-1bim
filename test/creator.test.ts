...
describe('testando endpoints criador', ()=>{
    it('Deve inserir um criador no banco',async () => {
        ...
        expect(creatorMock.role).toBe(foundCreator?.role)
        expect(creatorMock.comics).toBe(foundCreator?.comics)
    })
    ...
    it('Deve atualizar um criador existente', async () => {
        ...
        expect(foundUpdatedCreator?.role).toBe(updatedCreatorMock.role);
